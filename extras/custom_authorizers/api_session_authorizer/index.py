...
redis_db = redis.StrictRedis(
    host='puppetsessions.kxrlrt.0001.use1.cache.amazonaws.com',
    port=6379,
    db=0,
...
def lambda_handler(event, context):
    _auth_token = event['authorizationToken']
    _method_arn = event['methodArn']

    _principal_id = redis_db.get(_auth_token)

    if not _principal_id:
        raise Exception('Unauthorized')

    else:
        redis_db.expire(_auth_token, 86400)

        _tmp = _method_arn.split(':')
        _api_gateway_arn_tmp = _tmp[5].split('/')
        _aws_account_id = _tmp[4]

        _policy = AuthPolicy(_principal_id, _aws_account_id)
        _policy.rest_api_id = _api_gateway_arn_tmp[0]
        _policy.stage = _api_gateway_arn_tmp[1]
        _policy.allow_all_methods()
